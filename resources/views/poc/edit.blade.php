            ...
            <h5>Previous Endorsements:</h5>
                <table class="striped centered">
                    <tr>
                        ...
                        <th class="center">Endorsement Type</th>
						<th class="center">Status</th>
                    </tr>
                   @forelse($showup->endorsements as $endorsement)
				   @if($endorsement->status != null)
						@if($endorsement->company_id == 0 && $endorsement->site_id == 0 && $endorsement->status != "JOB")
						<tr>
							<td>{{$endorsement->businesspartner != null ? $endorsement->businesspartner->company_name:"N / A"}}</td>
							<td>{{$endorsement->site != null ? $endorsement->site->site_name : "N / A"}}</td>
							<td>{{$endorsement->status ?:"N / A" }}</td>
							<td>{{$endorsement->endorsement_status ?:"N / A" }}</td>
						</tr>
						@elseif($endorsement->company_id != 0 && $endorsement->site_id != 0 && $endorsement->status == "JOB")
						<tr>
							<td>{{$endorsement->businesspartner != null ? $endorsement->businesspartner->company_name:"N / A"}}</td>
							<td>{{$endorsement->site != null ? $endorsement->site->site_name : "N / A"}}</td>
							<td>{{$endorsement->status ?:"N / A" }}</td>
						...
						@else
						<tr>
							<td colspan="4" class="center">No Previous Endorsements</td>
						</tr>
						@endif
...
<script>
			...
			$('#showupForm').submit(function(evt){
				...
				$("#info").html("");
					...
					let endorsement_type = $('select[name=endorsement_type]').val();
